...
export const useCalculator = () => {
	...
	const concatNumber = (valueButton: string) => {
		if (number.includes('.') && valueButton === '.') {
			return;
		}
		...
		if (number.startsWith('0') || number.startsWith('-0')) {
			...
			if (valueButton === '0' && number.includes('.')) {
				return setNumber(number + valueButton);
			}
			if (valueButton !== '0' && !number.includes('.')) {
				return setNumber(valueButton);
			}
			if (valueButton === '0' && !number.includes('.')) {
	...
	const calculate = () => {
		...
		if (lastOperation.current === OperatorsEnum.ADD) {
			setNumber(`${num1 + num2}`);
		}
		if (lastOperation.current === OperatorsEnum.SUBTRACT) {
			setNumber(`${num2 - num1}`);
		}
		if (lastOperation.current === OperatorsEnum.MULTIPLY) {
			setNumber(`${num1 * num2}`);
		}
		if (lastOperation.current === OperatorsEnum.DIVIDE) {
			setNumber(`${num2 / num1}`);
		}
